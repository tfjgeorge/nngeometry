...
class EmpiricalL2Loss:
    def __init__(self, model, dataloader):
        ...
        self.mods = self.get_individual_modules(model)

    def get_individual_modules(self, model):
         mods = []
         sizes_mods = []
         parameters = []
         start = 0
    ...
    def add_hooks(self, hook_x, hook_gy):
        handles = []
        for m in self.mods:
    ...
    def get_matrix(self, loss_closure):
        # add hooks
        self.handles += self.add_hooks(self.hook_savex, self.hook_compute_flat_grad)
    ...
    def hook_savex(self, mod, i):
        self.xs[mod] = i[0]

    def hook_compute_flat_grad(self, mod, grad_input, grad_output):
        mod_class = mod.__class__.__name__
        gy = grad_output[0]
        x = self.xs[mod]
        bs = x.size(0)
