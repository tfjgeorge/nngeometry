...
def test_jacobian_pushforward_dense_linear():
    for get_task in linear_tasks:
        ...
        check_tensors(
            output_after - output_before, doutput_lin.get_flat_representation().t()
        )
...
def test_jacobian_pushforward_implicit():
    for get_task in linear_tasks:
        ...
        check_tensors(
            doutput_lin_dense.get_flat_representation(),
...
def test_jacobian_pullback_dense():
    for get_task in linear_tasks:
        ...
        check_ratio(
            torch.dot(
                dw.get_flat_representation(), dinput_lin.get_flat_representation()
            ),
...
def test_jacobian_fdense_vs_pullback():
    for get_task in linear_tasks + nonlinear_tasks:
        for centering in [True, False]:
            ...
            n_output = FMat_dense.get_dense_tensor().size(0)
            ...
            frob_direct = (FMat_dense.get_dense_tensor() ** 2).sum() ** 0.5
...
def test_jacobian_eigendecomposition_pdense():
    for get_task in [get_small_conv_transpose_task]:
        for impl in ["eigh", "svd"]:
            ...
            check_tensors(
                pmat_dense.get_dense_tensor(), evecs @ torch.diag_embed(evals) @ evecs.T
            )
...
def test_jacobian_pdense_vs_pushforward():
    ...
    for get_task in linear_tasks + nonlinear_tasks:
        for centering in [True, False]:
            ...
            check_tensors(PMat_computed, PMat_dense.get_dense_tensor())
            ...
            check_tensors(
                Mv_pf_pb.get_flat_representation() / n,
...
def test_jacobian_pdense():
    for get_task in nonlinear_tasks:
        for centering in [True, False]:
            ...
            # Test get_diag
            check_tensors(
                torch.diag(PMat_dense.get_dense_tensor()), PMat_dense.get_diag()
            )

            # Test frobenius
            frob_PMat = PMat_dense.frobenius_norm()
            frob_direct = (PMat_dense.get_dense_tensor() ** 2).sum() ** 0.5
            ...
            check_tensors(
                dw.get_flat_representation(),
                dw_using_inv.get_flat_representation(),
                eps=5e-3,
            )
            ...
            check_tensors(
                dw.get_flat_representation(),
                PMat_inv.mv(PMat_dense.mv(dw) + regul * dw).get_flat_representation(),
                eps=5e-3,
            )
            ...
            check_tensors(
                PMat_dense.get_dense_tensor() + PMat_dense2.get_dense_tensor(),
...
def test_jacobian_pdiag_vs_pdense():
    for get_task in nonlinear_tasks:
        ...
        check_tensors(mv_direct, mv_PMat_diag.get_flat_representation())
        ...
        check_tensors(
            PMat_diag.get_dense_tensor() + PMat_diag2.get_dense_tensor(),
        ...
        check_tensors(
            1.23 * PMat_diag.get_dense_tensor(), (1.23 * PMat_diag).get_dense_tensor()
        )
...
def test_jacobian_pblockdiag():
    for get_task in nonlinear_tasks:
        ...
        check_tensors(mv_direct, PMat_blockdiag.mv(dw).get_flat_representation())

        # Test vTMV
        check_ratio(
            torch.dot(mv_direct, dw.get_flat_representation()), PMat_blockdiag.vTMv(dw)
        )
        ...
        check_tensors(
            dw.get_flat_representation(),
            PMat_inv.mv(PMat_blockdiag.mv(dw) + regul * dw).get_flat_representation(),
            eps=5e-3,
        )
        ...
        check_tensors(
            PMat_blockdiag.get_dense_tensor() + PMat_blockdiag2.get_dense_tensor(),
            (PMat_blockdiag + PMat_blockdiag2).get_dense_tensor(),
        )
        check_tensors(
            PMat_blockdiag.get_dense_tensor() - PMat_blockdiag2.get_dense_tensor(),
            (PMat_blockdiag - PMat_blockdiag2).get_dense_tensor(),
...
def test_jacobian_plowrank():
    for get_task in nonlinear_tasks:
        ...
        # Test vTMV
        check_ratio(
            torch.dot(mv_direct, dw.get_flat_representation()), PMat_lowrank.vTMv(dw)
        )
...
def test_jacobian_pquasidiag():
    for get_task in [get_conv_task, get_fullyconnect_task]:
        ...
        mv = PMat_qd.mv(v)
        check_tensors(torch.mv(dense_tensor, v_flat), mv.get_flat_representation())
...
def test_example_passing():
    ...
    for get_task in [get_fullyconnect_task]:
        ...
        check_tensors(
            PMat_dense.get_dense_tensor(),
            (1.0 / tot_examples * sum_mats).get_dense_tensor(),
        )
